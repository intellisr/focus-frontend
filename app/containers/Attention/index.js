...
export function Attention() {
  ...
  useEffect(() => {
    const loadModels = async () => {
      ...
      // const model2 = poseDetection.SupportedModels.BlazePose;
      ...
      Promise.all([
        //poseDetector = await poseDetection.createDetector(model2, detectorConfig2),
  ...
  async function findAttention(image) {
    // const estimationConfig = {enableSmoothing: true};
    // const poses = await poseDetector.estimatePoses(image, estimationConfig);
    // console.log(poses)
  ...
  function create_training_data(x){
    const imageSrc = webcamRef.current.getScreenshot({ width: 640, height: 480 });
    if (!imageSrc) return;
    ...
    return new Promise((resolve) => {
      ...
      imgRef.current.onload = () => {
        try {
          if(x){
            stayAttention(imgRef.current)
          }else{
           stayNoAttention(imgRef.current)
          }
          readData();
          resolve();
        } catch (error) {
  ...
  async function stayAttention(image) {
    const faces = await landmarkDetector.estimateFaces(image);
    const data=filter(faces);
    writeUserData(data,"true")
  }

  async function stayNoAttention(image) {
    const faces = await landmarkDetector.estimateFaces(image);
    const data=filter(faces);
    writeUserData(data,"false")
  }
  ...
  function writeUserData(data, label) {
    let r = (Math.random() + 1).toString(36).substring(2);
    const db = getDatabase();
    set(ref(db, 'dataset/'+r), {
      data: data,
      label: label,
    });
  }
  ...
  return (
    <div>
      <h2>Real-time Landmark Detection</h2>
      <div style={{ display: 'flex', width: "100%", height: "90%",justifyContent:'center' }}>
        ...
        <div style={{ alignItems: 'flex-end' }}>
          ...
          />
          <button onClick={()=>create_training_data(true)}>Create Training data(Attention)</button>
          <br/>
          <br/>
          <button onClick={()=>create_training_data(false)}>Create Training data(Not Attention)</button>
          <br/>
          <br/>
          <button onClick={()=>readData()}>Show DataSet</button>
          </div>
      </div>
...
export default compose(withConnect)(Attention);
